...
public class ConnectionHelper {
        ...
        new Timer().scheduleAtFixedRate(new TimerTask() {
            ...
            public void run() {
                ...
                if (preferredNeighbors.size() > 1) {
                    ...
                    broadcaster.addMessage(new Object[] { conn, MessageHelper.Type.CHOKE, Integer.MIN_VALUE });
            ...
            public void run() {
                for (Connection conn : allConnections) {
                    if (!notInterested.contains(conn) && !preferredNeighbors.contains(conn) && !conn.hasFile()) {
                        broadcaster.addMessage(new Object[] { conn, MessageHelper.Type.UNCHOKE, Integer.MIN_VALUE });
    ...
    public synchronized void addInterestedConnection(String peerId, Connection connection) {
        if (preferredNeighbors.size() <= k && !preferredNeighbors.contains(connection)) {
            ...
            broadcaster.addMessage(new Object[] { connection, MessageHelper.Type.UNCHOKE, Integer.MIN_VALUE });
        }
